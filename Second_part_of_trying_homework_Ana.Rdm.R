...
library(msigdbr)
msigdr <- msigdbr(species = "Homo sapiens", category = "H")

data("escape.gene.sets", package="escape")
gene.sets <- escape.gene.sets
gene.sets <- list(Retinalcell_signature = c("PO4F2","CYP1B1")
...
seurat_ex <- AddMetaData(pbmc, ES)
seurat_ex@meta.data$active.idents <- seurat_ex@active.ident

ES2 <- data.frame(seurat_ex[[]], Idents(seuart_ex))
colnames(ES2)[ncol(ES2)] <- "cluster"

And this is where I gave up, I do not know how to make ridge plots :(
